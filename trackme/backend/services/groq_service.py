...
A mentee wrote their daily learning log. Restructure it into a clean professional entry.

Return ONLY valid JSON, no markdown:
{{
  "title": "Clear professional title (e.g. 'Introduction to Docker Containers')",
  "topics": ["3-6 short topic tags"],
  "structured_content": "Structured log under 1200 characters. Sections: What I Learned | Key Concepts | Challenges | Next Steps. Keep the mentee's meaning. Use professional language without losing their voice."
}}
...
async def detect_difficulty(
    ...
    if mentee_hint and mentee_hint.lower() in valid_levels:
        hint_clause = (
            f"\nMentee self-reported: {mentee_hint.upper()}. "
    ...
    def _call():
        return client.chat.completions.create(
            ...
            messages=[{
                "role": "user",
                "content": f"""Classify the difficulty of this learning log.

Respond with EXACTLY one word: beginner, intermediate, or advanced.

beginner — definitions, first exposure, surface-level understanding
intermediate — explains HOW/WHY, understands mechanisms, can apply concepts
advanced — system design, architectural trade-offs, deep implementation detail
{hint_clause}
...
_BLOOM_GUIDE = {
    "beginner": {
        ...
        "instruction": (
            ...
            "at a basic level — not just what it's called. They should give "
    ...
    "intermediate": {
        ...
        "instruction": (
            ...
            "They should reason through a real decision, not recall a definition. "
...
Write ONE question that tests whether they actually understood what they studied.
...
Before writing: identify the domain, the trickiest concept in the log, and what real-world context fits.

Question rules:
- Feel like a message from someone who actually read the log — not an exam setter
- Drop the mentee into a realistic situation in THEIR domain
- ONE clear question that requires thinking, not recall
- Under 100 words total
- Vary the opening — don't always lead with praise or a fixed phrase

Bad (robotic): "Oh nice, you got into X today. Anyway, I've got one for you — you're a backend engineer..."
Good (natural): "Quick one — you're three days into onboarding and the creative director asks you to walk the team through [concept]. The junior has zero context. How do you frame it?"
Good (direct): "Your client just rejected the [concept] approach because 'it feels complicated.' The simpler option has a real tradeoff. What do you tell them?"
Good (observation-led): "The tricky part about [concept] is that most people get it wrong the same way. What's the mistake, and what does getting it right look like?"

Difficulty: {difficulty.upper()}
Test for: {bloom["instruction"]}
Frame: {frame["label"]} — {frame["instruction"]}
...
_SCORING_RUBRIC = {
    ...
    "intermediate": {
        ...
        "dimensions": [
            ...
            ("Application to the scenario", 40, "Did they engage with the specific situation?"),
...
async def evaluate_answer(
    ...
    if question_type == "reflection":
        eval_instruction = """This is a reflection question — no single correct answer.
Evaluate on:
- Honesty and self-awareness (40 pts): Genuine reflection or safe/vague answer?
- Specificity (35 pts): Did they name something concrete — a concept, a gap, a moment of confusion?
- Growth signal (25 pts): Does the answer show they're thinking about their own learning?

Above 60 = meaningful reflection. Below 60 = too generic or surface-level.
Affirm genuine reflection. Gently push for more depth where missing."""
    else:
        eval_instruction = f"""Evaluate using these dimensions (total 100 pts):
{dimensions_text}

Pass threshold for {difficulty.upper()}: {threshold}/100
Pass: name exactly what they got right.
Fail: identify the exact gap — do NOT give the full answer away. Hint at what they missed."""

    prompt = f"""You are a senior technical mentor evaluating your mentee's answer.
Be fair, direct, specific. Generic feedback is useless.
...
Write THREE parts separated by blank lines:

Part 1 — Verdict (1 sentence): Be direct. "You got this." / "Close, but one thing is off." / "You missed the core of it." Don't sugarcoat a fail or undersell a pass.

Part 2 — Specific observation (2-3 sentences): Quote or paraphrase what they said. Pass = strongest part. Fail = specific gap. Never say "good effort" or "great attempt."

Part 3 — Feed-forward (1-2 sentences): What to do next — not what they got wrong, but what to do about it. Start with: "To sharpen this —" / "Next, go look at —" / "The thing to explore now is —"
...
Give an honest, data-grounded diagnostic. Every observation must be tied to something visible in the data.
...
- Longest gap: {longest_gap} days | Gaps > 2 days: {gap_count}
- Avg log word count: {avg_words} | Recent 5-log avg: {recent_avg}
- Depth declining (recent logs significantly shorter): {"YES — flag this" if depth_declining else "No"}
- Topic variety: {round(unique_topic_ratio * 100)}% unique — {"LOW — mentee may be stuck or avoiding new areas" if topics_repetitive else "Healthy"}
...
Framing:
- where_they_are_going = Feed-up: Are topics building toward a coherent goal?
- how_they_are_going = Feed-back: What does their actual trajectory show?
...
{{
  ...
  "overview": "4-5 sentences: learning trajectory, topic depth, consistency, test performance, one honest concern or strength grounded in data.",
  "recommendations": "3-4 specific actionable recommendations from THIS mentee's data. Each starts with a verb. No generic advice.",
  ...
  "risk_flags": ["flag tied to specific data point"],
  "strength_signals": ["strength tied to specific data point"],
  "session_agenda": ["agenda item tied to a gap or strength", "item 2", "item 3"],
  "where_they_are_going": "3-4 sentences: are topics building toward a coherent goal or scattered?",
  "how_they_are_going": "3-4 sentences: honest read of trajectory — depth, consistency, test results, log quality.",
  "what_mentor_should_do_next": "4-5 concrete feed-forward actions for this week. Each starts with a verb, references something specific in the data."
}}"""
...
async def generate_weekly_tasks(
    ...
    if mentee_logs:
        ...
        log_context = (
            f"Recent activity from {mentee_name} (avoid repeating covered ground, build on real momentum):\n"
            + "\n".join([
    ...
    if previous_incomplete:
        carry_context = (
            "\nIncomplete tasks from last week — carry forward (carried_over: true). Keep the intent, don't rewrite extensively:\n"
...
Turn the mentor's note into a clean focus plan. This is NOT a to-do list — each item is a meaningful area of work or learning to make real progress on by end of week. 3-5 strong items beats 8 shallow ones.

Use recent log context to avoid re-assigning covered topics and to build from where the mentee actually is.
...
---
EXAMPLE 1
Mentor input: "This week I want Tunde to get comfortable with how FastAPI handles authentication. He should understand JWT, how middleware works, and be able to protect routes properly."
Output:
{{
  "summary": "Get solid on FastAPI authentication — JWT, middleware, and protected routes",
  "tasks": [
    {{"title": "Understand JWT authentication flow", "description": "Learn how JWTs are issued, signed, and verified. Understand the difference between access and refresh tokens and when each is used.", "carried_over": false}},
    {{"title": "Implement FastAPI middleware for auth", "description": "Build and test middleware that intercepts requests and validates Bearer tokens before they hit protected endpoints.", "carried_over": false}},
    {{"title": "Protect routes and handle auth errors cleanly", "description": "Apply authentication dependencies to routes. Handle 401 and 403 responses properly with meaningful error messages.", "carried_over": false}}
  ]
}}

---
EXAMPLE 2
Mentor input: "Amara should finish the dashboard UI she started. Also wants her to look into how websockets work since we'll need it next sprint."
Output:
{{
  "summary": "Finish dashboard UI and get a working understanding of WebSockets",
  "tasks": [
    {{"title": "Complete the dashboard UI", "description": "Finish all incomplete components from last week — charts, filters, and the data table. Make sure it's connected to real API responses.", "carried_over": false}},
    {{"title": "Learn how WebSockets work", "description": "Understand the WebSocket protocol, how it differs from HTTP, and when to use it. Build a basic working example — even a simple chat or live counter.", "carried_over": false}}
  ]
}}
...
{{
  "summary": "...",
  "tasks": [
    {{"title": "...", "description": "...", "carried_over": false}}
  ]
}}"""
...
Tasks planned: {total} | Completed: {completed} ({completion_rate}%)
{"Incomplete: " + incomplete_titles if incomplete_titles else "All tasks completed."}

2-3 sentences. Rules:
- Honest about completion rate — don't sugarcoat below 70%
- Specific about what carries over if anything
- End forward-looking: what should the mentee do about it, not just what happened
- Written as mentor to mentee
...
async def generate_review_preview(
    ...
    logs_summary = "\n".join([
        f"- {l.get('structured_title', 'Untitled')} [{l.get('log_date', '?')}]: "
        f"{(l.get('structured_content') or '')[:200]}"
        for l in logs
    ]) or "No logs submitted this week."

    prompt = f"""Write a weekly review to send from mentor to mentee. Write for them, not about them.

Week: {focus['week_start']} to {focus['week_end']}
Goal: {focus['summary']}
Completion: {completion_rate}% ({completed}/{total} tasks)
...
Four paragraph sections — no bullet points, no task titles copied verbatim:

- summary: What actually happened this week. Brief and honest.
- progress: Direct read of completion rate. Don't sugarcoat below 60%. Don't over-celebrate 100%.
- recommendations: Feed-forward ONLY. What the mentee should DO next — specific actions, start with a verb, name concepts or skills explicitly.
- next_week_focus: What matters most going into next week and why. Ground it in what was incomplete or what momentum exists.
...
async def restructure_project_description(title: str, raw_description: str) -> str:
    ...
    def _call():
        return client.chat.completions.create(
            ...
            messages=[{
                "role": "user",
                "content": f"""Rewrite this project description for a mentorship platform.

Project: {title}
Description: {raw_description}

Rewrite as a clear, structured list of what the mentee is expected to learn or build.
Use specific technical terms. Make it scannable and comparable against daily logs.
Under 200 words. Plain text only — no JSON, no headers, no bullet symbols, clean sentences separated by newlines."""
            }],
            temperature=0.2,
            max_tokens=300,
        )
...
async def estimate_project_completion(
    ...
    prompt = f"""Evaluate a mentee's progress on a project based on their daily logs.
...
How much of the project scope has the mentee covered?
...
async def match_log_to_tasks(
    ...
    prompt = f"""Match a mentee's daily log to their weekly tasks.

Log title: {log_title}
Topics: {', '.join(log_topics)}
Content: {log_content[:500]}

Tasks:
{tasks_text}

Return IDs of tasks this log clearly covers. Only strong matches.
...
{mentee_name} is {days_behind} day(s) behind on Unit {unit_number}: "{unit_title}".
Incomplete: {', '.join(tasks_incomplete[:3]) if tasks_incomplete else 'multiple tasks'}

One sentence. State the delay clearly with the unit name. Direct but not alarming. End with a simple suggested action. Under 25 words.
