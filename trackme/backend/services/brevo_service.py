...
async def send_weekly_focus_set_email(
    mentee_email: str,
    mentee_name: str,
    mentor_name: str,
    summary: str,
    ...
    """Send email when a mentor sets a new weekly focus for a mentee"""
    first_name = mentee_name.split(" ")[0] if mentee_name else "there"
    ...
    if carried_over_count > 0:
        ...
        <div style="background:#FFFBEB;border:1px solid #FDE68A;border-radius:8px;
            ...
            <p style="color:#92400E;font-size:13px;margin:0;line-height:1.6;">
                📌 {carried_over_count} task(s) carried over from last week — keep pushing!
            </p>
        </div>"""
...
<body style="margin:0;padding:0;background:#F5F4FF;font-family:Urbanist,Arial,sans-serif;">
  <div style="max-width:560px;margin:40px auto;background:#fff;border-radius:16px;
    ...
    <div style="padding:36px 40px;">
      ...
      <p style="font-size:15px;color:#555;line-height:1.7;margin:0 0 24px;">
        Hey {first_name}, <strong>{mentor_name}</strong> just set your focus for
        <strong>{week_start}</strong> → <strong>{week_end}</strong>.
      </p>
      ...
      <div style="background:#F8F6FF;border:1.5px solid #E8E5FF;border-radius:12px;
                  padding:20px 24px;margin-bottom:16px;">
        <div style="font-size:10px;letter-spacing:2px;color:#7C3AED;font-weight:700;
                    text-transform:uppercase;margin-bottom:8px;">Focus Summary</div>
        <p style="font-size:14px;color:#0D0D0D;line-height:1.7;margin:0 0 12px;">{summary}</p>
        <div style="display:inline-flex;align-items:center;gap:6px;
                    background:#7C3AED;color:#fff;padding:6px 14px;
                    border-radius:20px;font-size:13px;font-weight:700;">
          📋 {task_count} tasks for this week
        </div>
      </div>
      
      <div style="background:#F0FDF4;border-radius:10px;padding:14px 18px;margin-bottom:28px;">
        <p style="color:#166534;font-size:13px;margin:0;line-height:1.6;">
          🔥 Stay consistent — log daily and crush those tasks!
        </p>
      </div>
    ...
    await send_email(
        to=mentee_email,
        subject=f"🎯 {mentor_name} set your weekly focus — {task_count} tasks",
        html=html
    )
