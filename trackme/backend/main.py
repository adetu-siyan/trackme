...
async def run_reminder_scheduler():
    ...
    while True:
        ...
        try:
            ...
            if current_time not in ("07:00", "13:00", "18:00"):
                continue

            # ── REMINDERS (2pm and 7pm Lagos) ─────────────────────
            if current_time in ("13:00", "18:00"):
                slot = "evening" if current_time == "18:00" else "morning"
                ...
                logger.info(f"[SCHEDULER] {current_time} UTC — sending {slot} reminders to {len(profiles)} users")
                ...
                try:
                    ...
                    for roadmap in (active_roadmaps.data or []):
                        ...
                        if current_unit["unit_number"] < expected_unit:
                            ...
                            mentee_res = supabase.table("profiles") \
                                .select("full_name").eq("id", roadmap["mentee_id"]).execute()
                            name = mentee_res.data[0]["full_name"] if mentee_res.data else "Your mentee"
