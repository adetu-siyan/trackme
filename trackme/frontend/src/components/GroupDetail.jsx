...
import { groupsApi } from '../lib/api'
import {
  ArrowLeft, UserPlus, CalendarDays, Users, FileText,
  CheckCircle, PenLine, Flame, BarChart2, TrendingUp,
  AlertTriangle, ChevronDown, X, RefreshCw, RotateCcw,
  CheckCheck, XCircle, Cpu
} from 'lucide-react'
import {
...
export default function GroupDetail({ group, onBack }) {
  ...
  useEffect(() => { loadDashboard() }, [group.id])
  ...
  async function handleExpandMentee(menteeId) {
    if (expandedMentee === menteeId) { setExpandedMentee(null); return }
    setExpandedMentee(menteeId)
    if (!menteeReview[menteeId]) {
      setLoadingReview(prev => ({ ...prev, [menteeId]: true }))
      try {
  ...
  function signalColor(signal) {
    return signal === 'Strong' ? 'var(--success)'
      : signal === 'Moderate' ? 'var(--accent)'
      : 'var(--danger)'
  }

  function signalBg(signal) {
    return signal === 'Strong' ? 'var(--success-soft)'
      : signal === 'Moderate' ? 'var(--accent-soft)'
      : 'var(--danger-soft)'
  }
    ...
    <div className="page" style={{ fontFamily: 'Urbanist, sans-serif' }}>
      <style>{`
        ...
        @media (max-width: 640px) {
          .group-header-inner { flex-direction: column !important; }
          .group-stats-grid { grid-template-columns: 1fr 1fr !important; }
          .group-action-btns { width: 100%; }
          .group-action-btns button { flex: 1; }
        }
      `}</style>
      ...
      <div style={{
        ...
        <div className="group-header-inner" style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', marginBottom: 8 }}>
              <div>
                ...
                {group.description && (
                  <p style={{
                    ...
                    margin: 0, lineHeight: 1.6, maxWidth: 340,
              ...
              <div className="group-action-btns" style={{ display: 'flex', gap: 10, flexShrink: 0 }}>
                <button
                  className="btn btn-secondary"
          ...
          {stats && (
            <div style={{
              ...
              {[
                ...
                { label: 'logged today', value: `${stats.logged_today}/${stats.total_members}`, Icon: FileText, color: stats.logged_today > 0 ? 'var(--success)' : 'var(--text-primary)' },
                { label: 'signed', value: stats.signed_today, Icon: CheckCheck },
                { label: 'log rate', value: `${stats.log_rate_today}%`, Icon: TrendingUp, color: stats.log_rate_today >= 80 ? 'var(--success)' : stats.log_rate_today >= 50 ? 'var(--warning)' : 'var(--danger)' },
              ].map((s, i) => (
                <div key={i} style={{
                  ...
                  <div style={{ fontSize: 22, fontWeight: 900, color: s.color || 'var(--accent)', marginBottom: 4 }}>
                    {s.value}
                  </div>
      ...
      {focusSuccess && (
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>
              {focusSuccess.week_start} → {focusSuccess.week_end}
            </div>
          </div>
          <button
            onClick={() => setFocusSuccess(null)}
        ...
        {[
          ...
          <button
            ...
            style={{
              ...
              border: tab === t.id ? '1.5px solid var(--accent)' : '1.5px solid var(--border)',
              cursor: 'pointer', fontFamily: 'Urbanist, sans-serif',
          ...
          {loadingDash ? (
            ...
            <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
              <div style={{
                ...
                <div>
                  <h3 style={{ margin: 0, marginBottom: 2 }}>Group Members</h3>
                  <p style={{ fontSize: 12, color: 'var(--text-muted)', margin: 0 }}>
                    {dashboard.mentees.length} mentee{dashboard.mentees.length !== 1 ? 's' : ''} · click a row to expand AI review
                  </p>
                </div>
                {dashboard.mentees.length > 0 && (
                  <button
              ...
              {dashboard.mentees.length === 0 ? (
                ...
                dashboard.mentees.map((m, i) => {
                  ...
                  return (
                    <div
                      key={m.mentee_id}
                      style={{ borderBottom: i < dashboard.mentees.length - 1 ? '1px solid var(--border)' : 'none' }}
                    >
                      {/* Row */}
                      <div
                        className="mentee-row"
                      ...
                      >
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                          <span style={{
                            ...
                            {m.has_logged_today
                              ? <><CheckCircle size={11} /> Logged</>
                              : <><XCircle size={11} /> Not logged</>
                            }
                          </span>
                          ...
                          <button
                            ...
                            style={{
                              ...
                              padding: 4, transition: 'color 0.15s', borderRadius: 6,
                            }}
                          >
                            <X size={14} />
                          </button>
                      ...
                      {isExpanded && (
                        <div style={{
                          ...
                          {isLoadingReview ? (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                              {[80, 60, 90, 70].map((w, i) => (
                                <div key={i} className="skeleton" style={{ height: 13, width: `${w}%`, borderRadius: 8 }} />
                              ))}
                            </div>
                          ) : review ? (
                              ...
                              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                                {review.ai_overview?.consistency_signal && (
                                  <span style={{
                                    ...
                                    background: signalBg(review.ai_overview.consistency_signal),
                                    color: signalColor(review.ai_overview.consistency_signal),
                                  }}>
                                    <RotateCcw size={11} />
                                    {review.ai_overview.consistency_signal}
                                  </span>
                                    ...
                                    {review.recent_logs.slice(0, 5).map(log => (
                                      <div key={log.id} style={{
                                        ...
                                        {log.signed
                                          ...
                                          : log.sent_to_mentor
                                          ? <PenLine size={14} color="var(--warning)" />
        ...
        <div>
          {loadingAnalytics ? (
            ...
            <div style={{ textAlign: 'center', padding: '60px 20px' }}>
              ...
              >
                <BarChart2 size={14} /> Run Analytics
              </button>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              {/* Summary stat cards */}
              <div className="group-stats-grid" style={{
                display: 'grid',
                gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))',
                gap: 12,
              }}>
                {[
                  { label: 'Total Members', value: analytics.group_summary.total_members, Icon: Users },
                  { label: 'Avg Sign Rate', value: `${analytics.group_summary.avg_sign_rate}%`, Icon: PenLine, color: analytics.group_summary.avg_sign_rate >= 70 ? 'var(--success)' : analytics.group_summary.avg_sign_rate >= 40 ? 'var(--warning)' : 'var(--danger)' },
                  { label: 'Avg Streak', value: analytics.group_summary.avg_streak, Icon: Flame },
                  { label: 'At Risk', value: analytics.group_summary.at_risk_count, Icon: AlertTriangle, color: analytics.group_summary.at_risk_count > 0 ? 'var(--danger)' : 'var(--success)' },
                  { label: 'Doing Well', value: analytics.group_summary.doing_well_count, Icon: CheckCircle, color: 'var(--success)' },
                ].map((s, i) => (
                  <div key={i} className="card" style={{ textAlign: 'center', padding: '16px 12px' }}>
                    <s.Icon size={18} color={s.color || 'var(--accent)'} style={{ margin: '0 auto 8px' }} />
                    <div style={{ fontSize: 22, fontWeight: 900, color: s.color || 'var(--accent)', marginBottom: 4 }}>
                      {s.value}
                    </div>
                    <div style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
              ...
              <div className="card">
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={analytics.mentees.map(m => ({
                    ...
                    <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" />
                    <XAxis dataKey="name" tick={{ fontSize: 12, fill: 'var(--text-muted)', fontFamily: 'Urbanist' }} />
                    <YAxis tick={{ fontSize: 12, fill: 'var(--text-muted)', fontFamily: 'Urbanist' }} domain={[0, 100]} />
                    <Tooltip contentStyle={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, fontFamily: 'Urbanist' }} labelStyle={{ color: 'var(--text-primary)', fontWeight: 700 }} />
                    <Bar dataKey="rate" fill="#7C3AED" radius={[6, 6, 0, 0]} name="Sign Rate %" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
              ...
              {/* Streak chart */}
              <div className="card">
                <h3 style={{ marginBottom: 4 }}>Streaks</h3>
                <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 20 }}>
                  Current vs best streak per mentee
                </p>
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={analytics.mentees.map(m => ({
                    name: m.profile.full_name?.split(' ')[0] || 'Unknown',
                    current: m.streak.current_streak,
                    best: m.streak.longest_streak,
                  }))}>
                    <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" />
                    <XAxis dataKey="name" tick={{ fontSize: 12, fill: 'var(--text-muted)', fontFamily: 'Urbanist' }} />
                    <YAxis tick={{ fontSize: 12, fill: 'var(--text-muted)', fontFamily: 'Urbanist' }} />
                    <Tooltip contentStyle={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, fontFamily: 'Urbanist' }} labelStyle={{ color: 'var(--text-primary)', fontWeight: 700 }} />
                    <Bar dataKey="current" fill="#059669" radius={[6, 6, 0, 0]} name="Current Streak" />
                    <Bar dataKey="best" fill="#7C3AED" radius={[6, 6, 0, 0]} name="Best Streak" opacity={0.4} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
              ...
              <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
                <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)' }}>
                  <h3 style={{ margin: 0, marginBottom: 3 }}>Individual Performance</h3>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 12 }}>
                        ...
                        <div style={{ textAlign: 'right', flexShrink: 0 }}>
                          <div style={{
                            ...
                            color: m.stats.sign_rate >= 70 ? 'var(--success)' : m.stats.sign_rate >= 40 ? 'var(--warning)' : 'var(--danger)',
                          }}>
                            {m.stats.sign_rate}%
                          </div>
                          <div style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 600, letterSpacing: '0.5px' }}>
                            SIGN RATE
                          </div>
                        </div>
                      </div>
        ...
        >
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
              ...
              <h2 style={{ margin: 0 }}>Set Group Weekly Focus</h2>
            </div>
            <p className="text-muted" style={{ fontSize: 13, marginBottom: 12 }}>
              Generates a personalized weekly plan for <strong>every member</strong> of this group.
            </p>
            <div style={{
              ...
              <Cpu size={14} color="var(--accent)" style={{ flexShrink: 0, marginTop: 1 }} />
              AI will cross-reference each mentee's recent logs and carry over any incomplete tasks from last week.
            </div>
            <form onSubmit={handleSetFocus} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <textarea
