            ...
            {loading ? (
              ...
              <div style={{ textAlign: 'center', padding: '80px 20px' }}>
                ...
                <h3 style={{ marginBottom: 8, color: 'var(--text-secondary)' }}>
                  {mentees.length === 0 ? 'No mentees yet' : `No results for "${search}"`}
                </h3>
                <p className="text-muted" style={{ fontSize: 14 }}>
                  {mentees.length === 0 ? 'Share your email with someone and ask them to add you as their mentor.' : 'Try adjusting your search.'}
                </p>
              </div>
            ) : (
              <div className="mentee-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
                        ...
                        <div style={{ 
                          ...
                          <span style={{ fontSize: 10, fontWeight: 600, color: activityColor, whiteSpace: 'nowrap' }}>
                            {daysSinceLog === null ? 'Never' : daysSinceLog === 0 ? 'Today' : `${daysSinceLog}d ago`}
                          </span>
                        </div>
                      </div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 16px', background: 'var(--surface-2)', borderRadius: 12, padding: '12px 14px' }}>
                        <div>
                          ...
                          <div style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Logs</div>
                        </div>
                        <div>
                          <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--text-primary)' }}>{stats.signed_logs || 0}</div>
                          <div style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Signed</div>
                        </div>
                        <div>
                          <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--text-primary)' }}>{stats.sign_rate || 0}%</div>
                          <div style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Rate</div>
                        </div>
                        <div>
                          <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--text-primary)' }}>{streak.current_streak || 0}🔥</div>
                          <div style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Streak</div>
                        </div>
                      </div>
