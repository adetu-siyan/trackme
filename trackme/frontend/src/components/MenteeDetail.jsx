...
export default function MenteeDetail({ mentee, onBack }) {
  ...
  return (
    <div className="page">
      ...
      <div className="mentee-stats-grid">
        {[
          { icon: <FileText size={18} />, label: 'Total Logs', value: stats.total_logs || 0 },
          { icon: <PenLine size={18} />, label: 'Signed', value: stats.signed_logs || 0 },
          { icon: <BarChart2 size={18} />, label: 'Sign Rate', value: `${stats.sign_rate || 0}%` },
          { icon: <Flame size={18} />, label: 'Streak', value: streak.current_streak || 0 },
          { icon: <Trophy size={18} />, label: 'Best Streak', value: streak.longest_streak || 0 },
          { icon: <Clock size={18} />, label: 'Most Active', value: overview?.stats?.most_active_time || (loading ? '...' : '—') },
        ].map((stat, i) => (
          <div key={i} className="card" style={{ textAlign: 'center', padding: '16px 12px' }}>
            <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 6, color: 'var(--accent)' }}>
              {stat.icon}
            </div>
            <div style={{ fontSize: 20, fontWeight: 900, color: 'var(--accent)', marginBottom: 4 }}>
              {stat.value}
      ...
      <div className="card" style={{ marginBottom: 20, padding: 0, overflow: 'hidden' }}>
        <div style={{
          ...
          <h3 style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <Calendar size={17} /> This Week's Focus
          </h3>
          {currentFocus && (
            <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>
              {currentFocus.week_start} → {currentFocus.week_end}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            {(ai.risk_flags?.length > 0 || ai.strength_signals?.length > 0) && (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                {ai.risk_flags?.length > 0 && (
                  <div style={{ padding: '14px 16px', borderRadius: 10, background: 'var(--danger-soft)', border: '1px solid rgba(220,38,38,0.15)' }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--danger)', letterSpacing: '1px', textTransform: 'uppercase', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 5 }}>
                ...
                {ai.strength_signals?.length > 0 && (
                  <div style={{ padding: '14px 16px', borderRadius: 10, background: 'var(--success-soft)', border: '1px solid rgba(5,150,105,0.15)' }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--success)', letterSpacing: '1px', textTransform: 'uppercase', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 5 }}>
            ...
            {ai.session_agenda?.length > 0 && (
              <div style={{ padding: '14px 16px', borderRadius: 10, background: 'var(--surface-2)', border: '1px solid var(--border)' }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '1px', textTransform: 'uppercase', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 5 }}>
                  <ClipboardList size={11} /> Next Session Agenda
                </div>
                {ai.session_agenda.map((item, i) => (
                  <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', padding: '8px 0', borderBottom: i < ai.session_agenda.length - 1 ? '1px solid var(--border)' : 'none' }}>
                    <span style={{ width: 22, height: 22, borderRadius: '50%', flexShrink: 0, background: 'var(--accent-soft)', color: 'var(--accent)', fontSize: 11, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                ...
                <div key={log.id} style={{ borderBottom: i < logs.length - 1 ? '1px solid var(--border)' : 'none' }}>
                  ...
                  >
                    <div style={{
                      ...
                      {isSigned
                        ...
                        : isSent
                        ? <MailOpen size={18} color="var(--warning)" />
                  ...
                  {isExpanded && (
                    <div style={{
                      ...
                      {isSigned && log.signed_at && (
                        <div style={{ marginTop: 12, fontSize: 12, color: 'var(--success)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 5 }}>
                          <PenLine size={13} /> Signed {formatDate(log.signed_at)}
                        </div>
                      )}
                    </div>
                  )}
          ...
          >
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                ...
                >
                  {editingReview ? <><Eye size={12} /> Preview</> : <><Pencil size={12} /> Edit</>}
                </button>
                <button
                  onClick={() => { setShowReviewModal(false); setEditingReview(false) }}
                  style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', display: 'flex' }}
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ fontSize: 12, color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 5 }}>
                {editingReview && <><Pencil size={11} /> Your edits will be sent exactly as written</>}
              </div>
              <div style={{ display: 'flex', gap: 10 }}>
                <button className="btn btn-secondary" onClick={() => { setShowReviewModal(false); setEditingReview(false) }}>
                  Cancel
                ...
                >
                  {sendingReview
                    ? 'Sending...'
                    : <><Mail size={14} /> Send to {profile.full_name?.split(' ')[0]}</>}
                </button>
              </div>
            </div>
          </div>
